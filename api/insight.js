  ...
  try {
    ...
    const prompt = {
      ...
      rules: [
        ...
        "If facts are insufficient to explain the move, say 'No clear single driver' and focus on what IS known (e.g., upcoming earnings, recent headlines, broader market context if present).",
    ...
    const response = await openai.responses.create({
      model: "gpt-4o-mini",
      input: [
        { role: "system", content: "You write concise, finance-style market narratives. Follow the rules exactly." },
        { role: "user", content: JSON.stringify(prompt) }
      ],
      response_format: {
        type: "json_schema",
        json_schema: { name: "insight", strict: true, schema }
      }
    });
